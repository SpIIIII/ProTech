...
class Plot:
    ...
    def draw_plot_for_month(self,frame,punkt_name, month_name = 'Январь'):
        ...
        with plt.style.context('seaborn-white'):
            ...
            for i in range(len(self.regular_punkts_quantity)):
                ax.bar3d(self.days_of_week[i], self.week_of_month[i], 0, 0.3, 0.3, self.regular_punkts_quantity[i], color='paleturquoise')
                ax.bar3d(self.days_of_week[i], self.week_of_month[i], self.regular_punkts_quantity[i], 0.3, 0.3, self.annual_punkts_quantity[i], color='orange', alpha = self.alpha[i],zsort='max')
            ...
            # ani = animation.FuncAnimation(fig, self.draw_plot_for_month, interval=100, blit=False)