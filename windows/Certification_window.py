import tkinter as tk
from tkinter import ttk

class Certification(tk.Toplevel):
    def __init__ (self, main):
        super().__init__ (main)
        self.main=main
        self.certifications = self.main.Certifications
        self.Punkts = self.main.Punkts
        self.bind('<Escape>', lambda e: self.destroy())
        self.title("Паспортизация")
        self.geometry("600x350")
        self.minsize(600,350)
        self.resizable(False,False)
        self.init_certification_window()

        self.Month_assoc = {'Январь':0, 'Февраль':1, 'Март':2, 'Апрель':3, 'Май':4, 'Июнь':5, 'Июль':6, 'Август':7,
                                                        'Сентябрь':8, 'Октябрь':9, 'Ноябрь':10, 'Декабрь':11}
        self.Month_assoc_back = {0:'Январь', 1:'Февраль', 2:'Март', 3:'Апрель', 4:'Май', 5:'Июнь', 6:'Июль', 7:'Август',
                                                        8:'Сентябрь', 9:'Октябрь', 10:'Ноябрь', 11:'Декабрь'}

    def init_certification_window(self):
        # Draw Frames
        self.top_frame = ttk.Frame(self, height=68)
        self.top_frame.pack(side=tk.TOP, fill=tk.BOTH, expand=True)

        self.bottom_frame = ttk.Frame(self)
        self.bottom_frame.pack(side=tk.BOTTOM, fill=tk.BOTH, expand=True)

        self.fill_top_frame()
        self.fill_bottom_frame()
         # Draw MenuBar
        menubar = tk.Menu()
        filemenu = tk.Menu(menubar, tearoff=0)
        
        filemenu.add_command(label="Добавить", command=lambda: self.fill_top_frame(version=1))
        filemenu.add_separator()
        filemenu.add_command(label="Выход", command=self.destroy)
        
        menubar.add_cascade(label="Файл", menu=filemenu)
        self.config(menu=menubar)

        #fill top_frame

        self.grab_set()
        self.focus_set()

    def fill_bottom_frame(self):
        # create and fill tree view
        certs_tree = ttk.Treeview(self.bottom_frame, columns=('equipment', 'location', 'executer', 'month'), height=15, show='headings')

        certs_tree.column('equipment', width=60, anchor=tk.CENTER)
        certs_tree.column('location', width=60, anchor=tk.E)
        certs_tree.column('executer', width=200, anchor=tk.CENTER)
        certs_tree.column('month', width=100, anchor=tk.CENTER)

        certs_tree.heading('equipment', text='Оборудование')
        certs_tree.heading('location', text='Станция')
        certs_tree.heading('executer', text='Исполнитель')
        certs_tree.heading('month', text='Месяц')
        # certs_tree.bind('<Button-3>',self.drop_popup_menu)
        # certs_tree.bind("<Double-1>", self.show_punkt)
        certs_tree.pack(side=tk.BOTTOM, fill=tk.BOTH, expand=tk.YES)

    def fill_top_frame(self, version=0):
        for child in self.top_frame.winfo_children():
            child.destroy()
         # new sub frame that will contain all elements and then will be destroyed when viwe changed
        sub_top_frame = ttk.Frame(self.top_frame, height=68)
        sub_top_frame.pack(side=tk.TOP, fill=tk.BOTH, expand=True)
        if version == 0:
            pass
        elif version == 1:
            self.fill_top_frame_add(sub_top_frame)
            

    def fill_top_frame_add(self, sub_top_frame):
        # add equipment block
        equipment_label = ttk.Label(sub_top_frame, text="Оборудование") 
        equipment_label.place(x=10, y=10)
        equipment_entry = ttk.Entry(sub_top_frame)
        equipment_entry.place(x=10, y=30)

        # add station block
        location_label = ttk.Label(sub_top_frame, text="Станция") 
        location_label.place(x=110, y=10)
        location_entry = ttk.Entry(sub_top_frame)
        location_entry.place(x=110, y=30)

        # add executer block
        executer_label = ttk.Label(sub_top_frame, text="Исполнитель") 
        executer_label.place(x=210, y=10)
        executer_entry = ttk.Entry(sub_top_frame)
        executer_entry.place(x=210, y=30)

        # add conform button
        add_b = ttk.Button(sub_top_frame, text="Добавить")
        add_b.place(x=470, y=47)

        # sub function to calculation for month block
        def month_pair(one=None, two=None):
            if not two:
                two = 6
            if one:
                return (one, one+6)
            return (two-6, two)

        # function to add month block
        def post_month_comboboxes(start_pair):
            month_start_label = ttk.Label(sub_top_frame, text="Месяц") 
            month_start_label.place(x=330, y=10)
            month_start_combo = ttk.Combobox(sub_top_frame, values=['Январь','Февраль','Март','Апрель','Май','Июнь'], justify='center')
            month_start_combo.current(start_pair[0])
            month_start_combo.place(x=330, y=30)
            month_start_combo.bind('<<ComboboxSelected>>', lambda x:  post_month_comboboxes(month_pair(one=self.Month_assoc[month_start_combo.get()])))

            month_finish_combo = ttk.Combobox(sub_top_frame, values=['Июль','Август', 'Сентябрь','Октябрь','Ноябрь','Декабрь'], justify='center')
            month_finish_combo.current(start_pair[1]-6)
            month_finish_combo.place(x=330, y=50)
            month_finish_combo.bind('<<ComboboxSelected>>', lambda x:  post_month_comboboxes(month_pair(two=self.Month_assoc[month_finish_combo.get()])))
        
        # add month block
        post_month_comboboxes(month_pair())

    def fill_top_frame_regular(self):
        pass
        

